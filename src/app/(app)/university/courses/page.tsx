...
export default function UniversityPage() {
  ...
  const handleCreateCourse = async () => {
    if (!newCourseName.trim() || !newCourseCode.trim() || !newAccessCode.trim()) return
    ...
    try {
      course = await createCourse({
        ...
        access_code: newAccessCode.trim(),
          ...
          <GlassCard className="p-8 max-w-md w-full text-center">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <label className="text-sm text-[var(--text-muted)] text-left block">Class join code</label>
        ...
        <Dialog open={showCreateCourse} onOpenChange={setShowCreateCourse}>
          <DialogContent>
            ...
            <div className="mt-4 space-y-3">
              ...
              <div className="space-y-1">
                <label className="text-sm text-[var(--text-muted)]">Join code (students will use this)</label>
                <input
                  value={newAccessCode}
                  onChange={(e) => setNewAccessCode(e.target.value.toUpperCase().replace(/\\s+/g, ""))}
                  placeholder="e.g. FTF2026"
                  className="w-full rounded-xl bg-white/5 border border-white/10 px-4 py-2 text-sm text-white placeholder:text-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--theme-primary))]/50 font-mono"
                />
              </div>
              <div className="space-y-1">
            ...
            <DialogFooter className="mt-6">
              ...
              <Button
                ...
                disabled={creatingCourse || !newCourseName.trim() || !newCourseCode.trim() || !newAccessCode.trim()}
              >
  ...
  return (
    <PageContainer>
      ...
      {/* Create Course Dialog (also needed when user already has courses) */}
      <Dialog open={showCreateCourse} onOpenChange={setShowCreateCourse}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Create course</DialogTitle>
